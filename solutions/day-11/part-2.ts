...
readInFile('./inputs/day-11/input.dat', (data) => {
	...
	setup.forEach((entry) => {
		...
		const items = rules[1].match(NUMBER_PATTERN)?.map((item) =>
			Number(item)
		) ??
			[];
		...
		monkeys[position] = {
			...
			items,
			test: (item: number) =>
				item % testvalue === 0 ? trueTestMonkey : falseTestMonkey,
	...
	console.log(
		monkeys.map((m) => m.inspectionCount).sort((a, b) => b - a).slice(0, 2)
			.reduce((a, b) => a * b, 1),
	);
});
