...
const getDirectorySizes = (dir: Directory): DirectorySizes[] => {
  ...
  const subDirsSizes = dir.getDirectories().map((subDir) =>
    getDirectorySizes(subDir)
  ).reduce((a, b) => [...a, ...b], []);
  return [
    {
      name: dir.name,
      size: fileSize +
        subDirsSizes.map((sdr) => sdr.size).reduce((a, b) => a + b, 0),
    },
    ...subDirsSizes,
  ];
};
...
readInFile("./inputs/day-7/input.dat", (data) => {
  ...
  const size = directories.filter((size) => size.size < 100_000).map((dir) =>
    dir.size
  ).reduce((a, b) => a + b, 0);
  console.log(size);
});
