...
readInFile('./inputs/day-8/input.dat', (data) => {
	...
	for (
		...
		if (![0, gridWidth - 1].includes(index % gridWidth)) {
			...
			const left = leftBlockedIndex === -1
				? leftTrees.length
				: leftBlockedIndex + 1;
			const right = rightBlockedIndex === -1
				? rightTrees.length
				: rightBlockedIndex + 1;
			...
			const bottomBlockedIndex = bottomTrees.findIndex((bt) =>
				bt >= tree
			);

			const top = topBlockedIndex === -1
				? topTrees.length
				: topBlockedIndex + 1;
			const bottom = bottomBlockedIndex === -1
				? bottomTrees.length
				: bottomBlockedIndex + 1;
